{ pkgs, postgresql, writeShellScriptBin } :
...
let
  ...
  ver = "16";  # builtins.head (builtins.splitVersion postgresql.version);
  script = ''
    set -euo pipefail
    ...
    ext_options="-c shared_preload_libraries=\"pg_cron\""

    pg_ctl start -o "$options" -o "$ext_options"

    psql -v ON_ERROR_STOP=1 -c "create extension pg_cron" -d postgres

    "$@"
  '';
in pkgs.writeShellScriptBin "psql-with-cron-${ver}" script