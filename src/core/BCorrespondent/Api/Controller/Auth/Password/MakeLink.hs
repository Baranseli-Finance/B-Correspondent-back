...
module BCorrespondent.Api.Controller.Auth.Password.MakeLink (controller, ResetPasswordLink) where
    ...
    Success -> do
      void $ fork $ do
        ...
        for_ cfg $ \(Sendgrid {..}, sendgrid) -> do
            let link = "https://BCorrespondent.app/#/auth/password/reset?key=" <> hash
            let reqBody = 
                  mkPOSTMailSendRequestBody 
