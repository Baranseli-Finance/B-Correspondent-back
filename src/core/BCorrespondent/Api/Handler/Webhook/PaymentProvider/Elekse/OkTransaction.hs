...
module BCorrespondent.Api.Handler.Webhook.PaymentProvider.Elekse.OkTransaction (handle) where
...
import BCorrespondent.Statement.Transaction (createOk, checkTransaction)
    ...
    withCheckRes (Right S.Ok) =
      fmap (const (Ok ())) $ do
        ...
        dbRes <- transactionM hasql $ do
          ...
          r <- statement createOk transaction
