...
module BCorrespondent.Api.Handler.Frontend.Dashboard.GetDailyBalanceSheet (handle) where
...
import Data.Time.Clock (UTCTime (utctDayTime), addUTCTime)
...
handle AuthenticatedUser {institution = Just ident} = do
  ...
  let lowerBound = addUTCTime (-3600) upperBound
